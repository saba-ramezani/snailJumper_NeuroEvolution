...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [3, 7, 2]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        for obstacle in obstacles:
            if obstacle['x'] < 180 and inputs[0] == 0:
                inputs[0] = player_y - obstacle['y']
            elif obstacle['x'] > 400 and inputs[2] == 0:
                inputs[2] = player_y - obstacle['y']
            elif inputs[1] == 0: #its a fly
                inputs[1] = (player_y - obstacle['y']) + np.abs(player_x-obstacle['x'])
